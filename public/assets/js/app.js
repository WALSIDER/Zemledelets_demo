...
function hydrateTagTooltips(scope = document) {
  scope.querySelectorAll(".tag").forEach((tag) => {
    ...
    const description = getTagDescription(text) || tag.dataset.tagDescription || "";
...
function getTagDescription(text) {
  ...
  const descriptions = {
    ...
    "экспорт": "Поставка сельскохозяйственной продукции на зарубежные рынки.",
    "официальный дилер": "Компания указывает статус официального представителя производителя по продаже продукции.",
    "сервис": "Техническое обслуживание, диагностика и ремонт сельхозтехники.",
    "выездной сервис": "Обслуживание и ремонт техники с выездом специалистов в хозяйство.",
    "закупки": "Компания закупает товары и услуги у поставщиков для своих производственных задач.",
    "сезонные контракты": "Поставки или обслуживание по договору на период сельскохозяйственного сезона.",
    "запросы цен": "Компания собирает цены и условия от поставщиков для выбора предложения.",
...
function getTagClass(type) {
  const companyTagClasses = {
    "зерно": "seeds",
    "экспорт": "export",
    "хранение": "infrastructure",
    "официальный дилер": "tech",
    "сервис": "service",
    "выездной сервис": "service",
    "запчасти": "tech",
    "закупки": "buy",
    "сезонные контракты": "contract",
    "запросы цен": "rfq",
    "проверено": "verified",
    "на модерации": "moderation"
  };
  const companyTagClass = companyTagClasses[normalizeText(type)];
  if (typeof companyTagClass === "string") {
    return companyTagClass;
  }
