...
function getEnterpriseSpecialties(company) {
  const statuses = new Set(["новая компания", "профиль на модерации", "на модерации", "проверено"]);
  return (Array.isArray(company.specialties) ? company.specialties : [])
    .filter((label) => typeof label === "string" && label.trim() && !statuses.has(normalizeEnterpriseText(label)));
}
...
function filterEnterpriseCompanies(items, filters) {
  ...
  return items.filter((company) => {
    const specialties = getEnterpriseSpecialties(company);
  ...
  return `
    <article class="enterprise_card white_card card_link_surface" data-href="${href}" tabindex="0" role="link" aria-label="Открыть компанию: ${escapeHtml(name)}">
      <div class="enterprise_card_top">
        ...
        <div class="enterprise_status">${renderEnterpriseTag(company.verified ? "Проверено" : "На модерации")}</div>
      </div>
...
function mountCompanyDirectory(items) {
  ...
  const resetButton = document.getElementById("companies-reset");
  let page = 1;
  const pageSize = 12;
  const regions = [...new Set(items.map((item) => item.region || item.city).filter(Boolean))].sort((a, b) => a.localeCompare(b, "ru"));
  const specialties = [...new Set(items.flatMap(getEnterpriseSpecialties))].sort((a, b) => a.localeCompare(b, "ru"));
  ...
  const render = () => {
    resetButton.hidden = !Boolean(search.value.trim() || region.value || specialty.value || verified.checked);
